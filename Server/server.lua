...
-- Function to check and update the daily reward status
function checkDailyReward(playerId)
    local identifier = GetPlayerIdentifier(playerId)

    -- Query the database to check the last reward claimed
    MySQL.Async.fetchScalar('SELECT lastClaimed FROM p42_drewards WHERE identifier = @identifier', {
        ['@identifier'] = identifier
    }, function(lastClaimed)
        if lastClaimed then
            -- Calculate the time difference in hours
            local hoursPassed = (os.time() - lastClaimed) / 3600
            if hoursPassed >= 24 then
                -- Unlock next reward
                TriggerClientEvent('unlockNextReward', playerId)
            end
        end
    end)
end
...
RegisterNetEvent('claimDailyReward')
AddEventHandler('claimDailyReward', function()
    local xPlayer = ESX.GetPlayerFromId(source)
    local identifier = xPlayer.getIdentifier()
    local currentTime = os.time()

    -- Update the reward claim in the database
    MySQL.Async.execute('REPLACE INTO p42_drewards (identifier, lastClaimed) VALUES (@identifier, @lastClaimed)', {
        ['@identifier'] = identifier,
        ['@lastClaimed'] = currentTime
    }, function(rowsChanged)
        if rowsChanged > 0 then
            -- Acknowledge the claim
            TriggerClientEvent('dailyRewardClaimed', source, currentTime)
        end
    end)
end)
...
AddEventHandler('esx:playerLoaded', function(playerId, xPlayer)
    -- Check and update the daily reward status
    checkDailyReward(playerId)
end)
